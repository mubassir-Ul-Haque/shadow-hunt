...
export class UIController {
  ...
  updateLobbyRoom(room, myId) {
    ...
    // Toggle Host Start Button vs Ready Button
    if (isHost) {
  ...
  updateHUD(snapshot, myId) {
    ...
    // Update Timer Display & Color transitions (Green -> Yellow -> Orange -> Red)
    ...
    // Update Alive Survivors & Killer Kills
  ...
  showPowerupToast(powerupType) {
    this.powerupToastText.textContent = `${powerupType.replace('_', ' ')} ACTIVATED!`;
  ...
  showMatchResults(data) {
    ...
    // Save match to LocalStorage Leaderboard
